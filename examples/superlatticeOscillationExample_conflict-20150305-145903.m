...
DL.addSubStructure(SRO,3);
DL.addSubStructure(STO,6);
...
semilogy(theta/u.deg, Rh, '-k', 'LineWidth', 2);
hold off
...
figure(5)
semilogy(theta/u.deg,RI(time == 1.5*u.ps, :), '-r', 'LineWidth', 2);hold on;
semilogy(theta/u.deg,RI(time == -1*u.ps, :), '-k', 'LineWidth', 2);
axis([theta(1)/u.deg theta(end)/u.deg 1e-7 1]);
grid off; box on;
xlabel('q_z [Ang^{-1}]');
ylabel('Reflectivity');
